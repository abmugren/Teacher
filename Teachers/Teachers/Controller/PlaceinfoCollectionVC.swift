...
class PlaceinfoCollectionVC: UIViewController {
    ...
    @IBOutlet weak var phoneshow: UIButton!
...
//        }
    ...
    override func viewWillAppear(_ animated: Bool) {
        if UserDefaults.standard.value(forKey: "id") == nil{
            ...
            totalfav.isHidden = true
        }else{
            favicon.isHidden = false
            ratebutton.isHidden = false
            totalfav.isHidden = false
        }
    ...
    @IBAction func faviconbtn(_ sender: Any) {
        if favicon.image(for: .normal) == #imageLiteral(resourceName: "like12"){
            ...
            WebServices.instance.RemovePlaceFromfavorite(placeID: placeID, completion: { (status,error ) in
                if status{
                    self.favicon.setImage( #imageLiteral(resourceName: "like12"),for: .normal)
                    print("Not Fav Done")
                    self.totalrate = self.totalrate - 1
                    self.totalfav.text = String(self.totalrate)
    ...
    override func viewDidLoad() {
            ...
            Alamofire.request("\(url)?id=\(placeID)", method: .get , encoding: URLEncoding.default).responseJSON { response in
                ...
                case .success(let value):
                    ...
                    if let category  = json["categoryID"].dictionary{
                        if APPLANGUAGE == "ar"{
                            self.category.text = (category["titleAr"]?.string)! + " / " + self.materialAR
                        }else{
                            self.category.text = (category["titleEN"]?.string)! + " / " + self.materialEN
                        }
                    ...
                    if let zone  = json["zoneID"].dictionary{
                        if APPLANGUAGE == "ar"{
                            self.addzone.text = (zone["titleAr"]?.string)!  + "/\(self.cityAr)"
                        }else{
                            self.addzone.text = (zone["titleEN"]?.string)! + " /\(self.cityEN) "
                        }
                    }
                    
                    if let urlPath = json["logo"].string{
                        if(urlPath.first != "h"){
                            self.logopic.image = UIImage(named: "defaultImage")
                        }
                        else {
                    ...
                    UserDefaults.standard.set(langit, forKey: "latit")
                    UserDefaults.standard.set(latit, forKey: "langit")
        ...
        materialTF.text = "material".localized
        contactUS.text = "contact".localized
        subTF.text = "sub".localized
