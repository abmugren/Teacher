...
class EditCitiesVC: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        if isConnectedToNetwork() == true {
            ...
            WebServices.instance.GetCities(id: zoneID) { (data) in
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        guard !searchText.isEmpty else {
            citiesArray = searchArray
            myTableView.reloadData()
            return
        }
        citiesArray = searchArray.filter({ leadInfo -> Bool in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (APPLANGUAGE == "en"){
            cell.textLabel?.text = citiesArray[indexPath.row].titleEN
        }
        else {
            cell.textLabel?.text = citiesArray[indexPath.row].titleAR
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if(APPLANGUAGE == "en"){
            newCityName = citiesArray[indexPath.row].titleEN!
        }
        else {
            newCityName = citiesArray[indexPath.row].titleAR!
        }
        
        newCityID = citiesArray[indexPath.row].id!
    ...
    @IBAction func doneBtn(_ sender: Any) {
        delegate.cities(cityName: newCityName, cityID: newCityID)
        dismiss(animated: true, completion: nil)
    }
}
